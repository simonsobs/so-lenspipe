...
with bench.show("NORM"):
    ...
    R_src_tt = pytempura.get_cross('MASK','TT',ucls,tcls,lmin,lmax,k_ellmax=mlmax)
    ...
    Nl_g_bh = solenspipe.bias_hardened_n0(Als[e1][0],Als['MASK'],R_src_tt[0]) * (ls*(ls+1.)/2.)**2.
...
with bench.show("QFUNC"):
    ...
    q_bh_1 = solenspipe.get_qfunc(px,ucls,mlmax,e1,Al1=Als[e1],est2=e2,Al2=Als[e2],R12=R_src_tt) 
    q_bh_2 = solenspipe.get_qfunc(px,ucls,mlmax,e1,Al1=Als[e1],est2=e2,Al2=Als[e2],R12=R_src_tt) 
...
if rank==0:
    ...
    with bench.show("Bin"):
        ...
        np.savetxt(f'{solenspipe.opath}mean_uicls_{e1}_{e2}.txt',uicls)
        np.savetxt(f'{solenspipe.opath}mean_uxcls_bh_1_{e1}_{e2}.txt',uxcls_bh_1)
        np.savetxt(f'{solenspipe.opath}mean_uacls_bh_{e1}_{e2}.txt',uacls_bh)
        np.savetxt(f'{solenspipe.opath}mean_noise{e1}_{e2}.txt',Nl_g_bh)
