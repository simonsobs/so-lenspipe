...
if args.ps_bias_hardening:
    rdn0= bias.rdn0(icov=0,alpha=polcomb,beta=polcomb,qfunc=qfunc,get_kmap=get_kmap,comm=comm,power=power,nsims=nsims,type='bh',ils=Als['L'], blens=blens, bhps=bhps, Alpp=Alpp, A_ps=A_ps)
    rdn0[nmax:] = 0
...
rdn0[nmax:] = 0
if args.curl:
    io.save_cols(f'{solenspipe.opath}/rdn0_curl_{polcomb}_{isostr}_{car}_{nsims}_{args.label}.txt',(Als['L'],rdn0[:nmax]))
elif args.ps_bias_hardening:
    io.save_cols(f'{solenspipe.opath}/rdn0_bh_{polcomb}_{isostr}_{car}_{nsims}_{args.label}.txt',(Als['L'],rdn0[:nmax]))
else:
    io.save_cols(f'{solenspipe.opath}/rdn0_{polcomb}_{isostr}_{car}_{nsims}_{args.label}.txt',(Als['L'],rdn0[:nmax]))
if rank==0:
    ...
    pl.add(ils,Nl)
    pl.add(ils,theory.gCl('kk',Als['L']))
